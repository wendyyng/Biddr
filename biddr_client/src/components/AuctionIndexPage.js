...
export default function AuctionIndexPage () {
  ...
  return (
    ...
    <div className="container mx-4 d-flex flex-column justify-content-center align-items-center">
        {
        auctions.map((a,i) => {
            return (
                <div key={i} className="shadow p-3 mb-5 bg-white rounded my-auto col-sm-12">
                    <h3 ><Link className="fw-bold text-dark text-decoration-none" to={`/auctions/${a.id}`}>{a.title}</Link></h3>
